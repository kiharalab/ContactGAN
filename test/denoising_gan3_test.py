...
class getDataset(Dataset):
    def __init__(self, lrd1, lrd2):
        ...
        self.lr3_imageFileNames = [os.path.join(lrd3, x) for x in os.listdir(lrd3)]
        self.lr1_imageFileNames.sort()
        self.lr2_imageFileNames.sort()
        self.lr3_imageFileNames.sort()
    ...
    def __getitem__(self,index):
        ...
        lr3_filename = self.lr3_imageFileNames[index]
        protein = self.nameNoPath[index].split(".")[0]
        lr1_rowdata=getLRitem(lr1_filename)       
        lr2_rowdata=getLRitem(lr2_filename)   
        lr3_rowdata=getLRitem(lr3_filename)   

        lr1_image = lr1_rowdata
        lr2_image = lr2_rowdata
        lr3_image = lr3_rowdata
        return lr1_image, lr2_image, lr3_image, protein
    
    def __len__(self):
        return len(self.lr1_imageFileNames)

lr1='./data/example_files/input/'
lr2='./data/example_files/input/'
lr3='./data/example_files/input/'
dataset = getDataset(lr1,lr2,lr3)
...
class Generator(nn.Module):
    ...
    def __init__(self, channels, no_of_dil_blocks):
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(3, self.channels, kernel_size=9, padding=4),
            nn.PReLU()
        )
